    ...
    <section class="content">
        <div class="container-fluid">
            <div class="row">
                ...
                @if (
                    !auth()->user()->hasInternship() &&
                        !$internship->isDeadlinePassed() &&
                        !auth()->user()->alreadyApplied($internship))
                    <div class="col-md-12">
                                        ...
                                        <ul>
                                            ...
                                            @if(auth()->user()->isEligibleToApply($internship))
                                             <button class="btn  btn-success float-right  ">
                                                    <a href="{{ route('userProfile.detail') }}" class="text-white text-decoration-none mb-3"> Fill Information</a>
                        ...
                        @else
                            <form action="{{ route('user.application.store', $internship->id) }}" method="post">
                                @csrf
                                ...
                                <div class="card card-outline card-primary">
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="col-md-6">
                                                ...
                                                <dl class="row">
                                                    ...
                                                    <dd class="col-sm-8">
                                                        {{ Auth::user()->information->first_name ? ucwords(Auth::user()->information->first_name) : '' }}
                                                        {{ Auth::user()->information->middle_name ? ucwords(Auth::user()->information->middle_name) : '' }}
                                                        {{ Auth::user()->information->last_name ? ucwords(Auth::user()->information->last_name) : '' }}
                                                    </dd>
                                                    <dt class="col-sm-4">Student ID: </dt>
                                                    <dd class="col-sm-8">
                                                        {{ Auth::user()->information->student_id ? Auth::user()->information->student_id : '-' }}
                                                    </dd>
                                                    <dt class="col-sm-4">Comulative CGPA: </dt>
                                                    <dd class="col-sm-8">
                                                        {{ Auth::user()->information->cgpa ? Auth::user()->information->cgpa : '-' }}
                                                    </dd>
                                                    <dt class="col-sm-4">University: </dt>
                                                    <dd class="col-sm-8">
                                                        {{ Auth::user()->information->university ? Auth::user()->information->university : '-' }}
                                                    </dd>
                                                </dl>
                                            </div>
                                            <div class="col-md-5">
                                                ...
                                                <ul>
                                                    @if (Auth::user()->information->application_letter_file_path)
                                                        <li><a target="_blank"
                                                                href="{{ asset('/uploads/application_acceptance/' . Auth::user()->information->application_letter_file_path) }}">
                                                                Application Letter</a></li>
                                                    @endif
                                                    @if (Auth::user()->information->application_acceptance_file_path)
                                                        <li><a target="_blank"
                                                                href="{{ asset('/uploads/application_acceptance/' . Auth::user()->information->application_acceptance_file_path) }}">
                                                                Application Acceptance Form </a> </li>
                                                    @endif
                                                    @if (Auth::user()->information->student_id_file_path)
                                                        <li><a target="_blank"
                                                                href="{{ asset('/uploads/application_acceptance/' . Auth::user()->information->student_id_file_path) }}">Student
                                                                ID</a></li>
                                                    @endif
                                                </ul>
                                            </div>
                                                    ...
                                                    @foreach ($internship->prerequisites as $prerequisites)
                                                        <div class="form-group">
                                                            <label>#{{ $loop->iteration }} )
                                                                {{ strpos('?', $prerequisites->pre_key) ? ucfirst($prerequisites->pre_key) : ucfirst($prerequisites->pre_key) . '?' }}</label>
                                    ...
                                    <div class="card-footer">
                                        <button type="submit" class="btn btn-success float-right"> <i
                                                class="fas fa-paper-plane mr-1"></i> Apply </button>
                                    </div>
                                </div>
                            </form>
                        @endif
